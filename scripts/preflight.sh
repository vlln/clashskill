...
_download_ui() {
    [ -f "$ZIP_UI" ] && return 0
    _okcat '⏳' "正在下载 Web UI..."
    local proxy_url="${URL_GH_PROXY:+${URL_GH_PROXY%/}/}${URL_CLASH_UI_DL}"
    curl \
        --progress-bar \
        --show-error \
        --fail \
...
_detect_init() {
    ...
    service_watch_proxy=(clashctl on)
    _is_regular_sudo && {
        service_watch_proxy=(_failcat "'未检测到代理变量，可执行 clashctl on 开启代理环境'")
        _SUDO=sudo
    }
...
$start_flag
# 加载代理环境（如果存在）
[ -f "${CLASH_BASE_DIR}/.proxy.env" ] && . "${CLASH_BASE_DIR}/.proxy.env"
# 加载 clashctl 命令
$source_clashctl
# 自动检测代理状态
watch_proxy
...
_revoke_rc() {
    ...
    # 清除代理环境文件
    rm -f "${CLASH_BASE_DIR}/.proxy.env" 2>/dev/null
}
